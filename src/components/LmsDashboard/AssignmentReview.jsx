...
export function AssignmentReview() {
  ...
  useEffect(() => {
    const fetchAssignmentDetails = async () => {
      setLoading(true);
      try {
        const response = await axios.get(`http://18.60.40.186:8081/api/assignment/${courseId}/${userId}`);
  ...
  const handleSaveScore = async (id) => {
    ...
    try {
      await axios.put(`http://18.60.40.186:8081/api/assignment/score/${id}`, { finalScore: itemToSave.tempScore });
      setAssignment(assignment.map(item =>
